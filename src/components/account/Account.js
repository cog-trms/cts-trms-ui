...
import { loadAccount, saveAccount } from '../../actions/account';
...
const Account = ({ account, loadAccount, saveAccount }) => {
  const businessUnit = {};
  const hiringManger = {};

  // account.forEach(ele => {
  //   const { id, buName } = ele.businessUnit;
  //   businessUnit[id] = buName;
  // });

  // account.forEach(ele => {
  //   const { id, fullName } = ele.hiringManger;
  //   hiringManger[id] = fullName;
  // });
  ...
  const handleUpdate = ({ id, accountName }) => {
    debugger;
    return updateBusiness(id, accountName);
  };
